...
{
    vkDestroyDescriptorPool(device->GetLogicalDeviceHandle(), descriptorPool,
                            nullptr);
}
...
void VulkanDescriptorPool::CreateDescriptorPool()
{
    ...
    VK_CALL(vkCreateDescriptorPool(device->GetLogicalDeviceHandle(),
...
VulkanDescriptorSet::VulkanDescriptorSet(VulkanDevice         *inDevice,
                                         VulkanDescriptorPool *inPool,
...
{
    VK_CALL(vkFreeDescriptorSets(device->GetLogicalDeviceHandle(),
                                 descriptorPool->GetHandle(), 1,
                                 &descriptorSet));
}